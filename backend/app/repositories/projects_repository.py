...
def get_projects_overview(db: Session) -> list[dict]:
    """Returnează date agregate per proiect pentru Dashboard.

    N+1 queries — acceptabil pentru <50 proiecte (scală tipică BIM).
    """
    projects = (
        db.query(ProjectModel)
    ...
    for p in projects:
        ...
        has_context = (
            db.query(ProjectContextModel)
            .filter(ProjectContextModel.project_id == p.id)
            .first()
        ) is not None

        # BEP generat?
        bep_doc = get_latest_generated_document(db, p.id, "bep")
        has_bep = bep_doc is not None
        bep_version = bep_doc.version if bep_doc else None

        # Verificări
        verifications = list_verification_reports(db, p.id)
